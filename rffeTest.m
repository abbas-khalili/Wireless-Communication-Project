...
isFixPoint = true;	% use low resolution PHY-layer processing
...
% Adjacent band to desired band power ratio
ratio = [0,1,sqrt(10),10];
for r= 1:length(ratio)
    ...
    for it = 1:nit
        ...
        tx = NRgNBTx(simParam, 'wtx', wtx, 'ncc', ncc,'isFixPoint', isFixPoint, ...
            'enableCA', enableCA,'nbits_dac', 6);
    ...
    figure(1)
    hold on
    plot(snrInTest,snrOut)
    grid on
    figure(2)
    hold on
    xfd = fftshift(fft(x));
    plot(20*log10(abs((xfd))));
    grid on;
    axis tight;
end
